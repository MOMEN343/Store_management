...
class SalesPage(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        inner.pack(pady=10) # Removed fill="x" to allow centering
        
        ctk.CTkLabel(inner, text=":تاريخ المبيعات", font=(FONT, 13, "bold"), text_color=self.C["accent"]).pack(side="right", padx=(10, 5))
        ctk.CTkLabel(inner, text="من", font=(FONT, 13), text_color=self.C["text2"]).pack(side="right", padx=(5, 2))
        ctk.CTkButton(inner, text="", width=36, height=36, fg_color=self.C["blue"], image=self._w_cal_icon, hover_color=self.C["accent"], corner_radius=8, command=lambda: self._pick_date_main(self._from_e)).pack(side="right", padx=(2, 5))
        ...
        ctk.CTkButton(inner, text="", width=36, height=36, fg_color=self.C["blue"], image=self._w_cal_icon, hover_color=self.C["accent"], corner_radius=8, command=lambda: self._pick_date_main(self._to_e)).pack(side="right", padx=(2, 5))
        ...
        # Added "Today's Sales" to the same group for centered layout
        ctk.CTkButton(inner, text="مبيعات اليوم", width=115, height=36, font=(FONT, 13, "bold"), image=self._w_cal_icon, compound="right", fg_color=self.C["blue"], text_color=self.C["btn_text"], hover_color="#1E40AF", corner_radius=8, command=self._today).pack(side="right", padx=(15, 0))
    ...
    def _pick_date_main(self, entry):
        _SaleDialog.show_calendar(self, entry, self.C)
