"""
صفحة المصروفات – CustomTkinter (النسخة الجمالية المطورة)
"""
...
class ExpensesPage(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(hdr, text="  سجل المصروفات", font=(FONT_HDR, 24, "bold"), image=self._exp_icon, compound="right", text_color=self.C["accent"]).pack(side="right")
        ctk.CTkButton(hdr, text="تسجيل مصروف جديد", font=(FONT, 15, "bold"), image=self._plus_icon, compound="right", fg_color=self.C["accent"], text_color=self.C["btn_text"], hover_color="#00897B", corner_radius=10, height=44, command=self._open_add).pack(side="left")
        ...
        self._kpi_total = self._make_kpi(kpi_frame, "إجمالي المصروفات", "₪ 0", get_icon("expenses", (28, 28)), self.C["danger"])
    ...
    def _setup_grid(self, frame):
        frame.grid_columnconfigure(0, weight=0, minsize=60)   # Delete (on far left)
        frame.grid_columnconfigure(1, weight=1)              # Description (fills space)
        frame.grid_columnconfigure(2, weight=0, minsize=140) # Amount
        frame.grid_columnconfigure(3, weight=0, minsize=140) # Date (on far right)

    def _make_kpi(self, parent, title, value, icon_img, color):
        c_map = { self.C["accent"]: self.C["accent_f"], self.C["danger"]: self.C["danger_bg"], self.C["blue"]: self.C["blue_f"] }
    ...
    def refresh(self):
        ...
        cols = [("تاريخ الصرف", 3), ("القيمة (₪)", 2), ("بيان المصروف / الملاحظات", 1), ("", 0)]
        ...
        if not rows:
            ctk.CTkLabel(self._scroll, text="لا توجد مصروفات مسجلة في هذه الفترة", font=(FONT, 16), text_color=self.C["text2"]).pack(pady=60)
            self._kpi_count.configure(text="0"); self._kpi_total.configure(text="₪ 0"); return
        ...
        for i, r in enumerate(rows):
            ...
            try: fmt_date = f"\u200E{datetime.strptime(r['expense_date'], '%Y-%m-%d').strftime('%d/%m/%Y')}"
            except: pass
            ctk.CTkLabel(row, text=fmt_date, font=(FONT, 14), text_color=self.C["text"], anchor="e").grid(row=0, column=3, padx=(0, 15), pady=12, sticky="nsew")

            # Amount (Col 2)
            ctk.CTkLabel(row, text=f"\u200E ₪ {r['amount']:,.2f}", font=(FONT, 14, "bold"), text_color=self.C["danger"], anchor="e").grid(row=0, column=2, padx=(0, 15), pady=12, sticky="nsew")
            
            # Description (Col 1)
            ctk.CTkLabel(row, text=r["description"], font=(FONT, 14), text_color=self.C["text"], anchor="e", justify="right", wraplength=480).grid(row=0, column=1, padx=(0, 15), pady=12, sticky="nsew")
            
            # Delete (Col 0)
            ctk.CTkButton(row, text="", width=32, height=32, fg_color="transparent", image=self._del_icon, hover_color=self.C["danger_bg"], corner_radius=8, command=lambda rid=r["id"]: self._del(rid)).grid(row=0, column=0, padx=15, pady=12, sticky="w")

        self._kpi_count.configure(text=str(len(rows)))
        self._kpi_total.configure(text=f"₪ {total:,.2f}")
    ...
    def _del(self, eid):
        ...
        if messagebox.askyesno("تأكيد الحذف", "هل تريد حذف هذا المصروف؟"):
            db.delete_expense(eid); self.refresh()

class _ExpDialog(ctk.CTkToplevel):
    def __init__(self, parent, colors, on_save=None):
        super().__init__(parent)
        self.C = colors; self._on_save = on_save
        self.title("تسجيل مصروف جديد"); self.geometry("480x500")
    ...
    def _build(self):
        ...
        ctk.CTkLabel(hdr_f, text="تسجيل مصروف جديد", font=(FONT_HDR, 22, "bold"), text_color="#FFFFFF", image=get_icon("expenses", (32, 32)), compound="right", anchor="center").pack(expand=True, fill="both")
        ...
        self._amount = add_field("قيمة المصروف (₪) *", "0.00")
        self._desc   = add_field("بيان المصروف / الوصف *", "مثلا: غداء عمال")
        self._date_e = add_field("تاريخ الصرف", "YYYY-MM-DD", has_cal=True)
        self._date_e.insert(0, date.today().isoformat())
        
        btn_row = ctk.CTkFrame(body, fg_color="transparent")
        btn_row.pack(fill="x", pady=(20, 0))
        self._save_btn = ctk.CTkButton(btn_row, text="💾 حفظ المصروف", font=(FONT, 16, "bold"), fg_color=self.C["accent"], text_color="#FFFFFF", hover_color="#00897B", corner_radius=12, height=46, command=self._save)
    ...
    def show_calendar(parent, entry, C):
        ...
        hdr.pack(fill="x", padx=10, pady=5); hdr.pack_propagate(False)
        def shift(m):
            if view_mode[0] == "days":
                curr_mo[0] += m
        ...
        def render():
            ...
            if view_mode[0] == "days":
                ...
                for c, d in enumerate(days): ctk.CTkLabel(main_fr, text=d, font=(FONT_REG, 11, "bold"), text_color=C["muted"]).grid(row=0, column=6-c, pady=(5, 10))
    ...
    def _save(self):
        try:
            ...
            db.add_expense(description=desc, amount=amt, category="عام", expense_date=d)
